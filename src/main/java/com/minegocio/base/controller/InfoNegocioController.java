 ...
 * 
 * int currentPage = page; int pageSize = size;
 * 
 * Page<InfoNegocio> infoNegocioPage = service.findPaginated(currentPage-1,
 * size);
 * 
 * model.addAttribute("infoNegocioPage",infoNegocioPage);
 * 
 * int totalPages = infoNegocioPage.getTotalPages(); if (totalPages > 0) {
 ...
 * " "+ConfigModulosMenus.base().nombre.toUpperCase());
 * model.addAttribute("menus", ConfigModulosMenus.base().menus);
 * model.addAttribute("titulo_cuerpo","Crear InfoNegocio"); return
 * "base/infonegocio/new"; }
 * 
 * @PostMapping("create") public String create(@ModelAttribute InfoNegocio
 * infoNegocio) { // ⑥
 * 
 ...
 * service.create(infoNegocio); System.out.println("ENTRO ACA");//desde el
 * create ya no imprime return "redirect:/base/infonegocio"; // ⑦ }
 * 
 * 
 * 
 * 
 ...
 * " "+ConfigModulosMenus.base().nombre.toUpperCase());
 * model.addAttribute("menus", ConfigModulosMenus.base().menus);
 * model.addAttribute("titulo_cuerpo","Datos de InfoNegocio"); InfoNegocio
 * infoNegocio = service.findById(id); model.addAttribute("infoNegocio",
 * infoNegocio); return "base/infonegocio/show"; }
 * 
 * }
 */
...
public class InfoNegocioController {
	...
	public String create(Model model) {
		model.addAttribute("modulo", " "+ConfigModulosMenus.base().nombre.toUpperCase());
		model.addAttribute("menus", ConfigModulosMenus.base().menus);
		model.addAttribute("titulo_cuerpo","Crear Negocio");
	...
	public String create(@ModelAttribute InfoNegocio infonegocio) {
		infonegocio.setActivo(true);
		//infonegocio.setNombre(infonegocio.getNombre().toUpperCase());
		infonegocio.setNombre(infonegocio.getNombre());
		System.out.println("llego aca"+infonegocio);
		//infonegocio.setContacto(infonegocio.getContacto());
		//infonegocio.setEmail(infonegocio.getEmail());
		//infonegocio.setTelefono(infonegocio.getTelefono());
		//falta la parte para cargar imagenes
